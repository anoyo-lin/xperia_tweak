...
%ADB% shell rm -f %TMPFOLDER%/debloat_lin.sh > NUL 2> NUL

%ADB% push %LISTFOLDER%/debloat_lin.sh %TMPFOLDER% > NUL 2> NUL
%ADB% shell chmod 0777 %TMPFOLDER%/debloat_lin.sh

echo.
echo Debloating (uninstalling quite a few packages, standby a minute or two). . .
%ADB% shell sh %TMPFOLDER%/debloat_lin.sh
